...
func (app *application) routes() http.Handler {
	...
	// create snippets
	mux.Get("/snippet/create", app.Session.Enable(app.isLoggedIn(noSurf(http.HandlerFunc(app.createSnippetForm)))))
	mux.Post("/snippet/create", app.Session.Enable(app.isLoggedIn(noSurf(http.HandlerFunc(app.createSnippet)))))
	// login
	...
	// log out
	mux.Post("/user/logout", app.Session.Enable(app.isLoggedIn(noSurf(http.HandlerFunc(app.logoutUser)))))
	// show snippet
	mux.Get("/snippet/:id", app.Session.Enable(app.isLoggedIn(noSurf(http.HandlerFunc(app.showSnippet)))))
